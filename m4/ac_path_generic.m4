...
#   LIBRARY_LIBS
#
#   The script must support `--cflags' and `--libs' args. If
#   MINIMUM-VERSION is specified, the script must also support the
#   `--version' arg. If the `--with-library-[exec-]prefix' arguments to
#   ./configure are given, it must also support `--prefix' and
#   `--exec-prefix'. (In other words, it must be like gtk-config.)
#
#   For example:
#
...
#   AM_PATH_GTK) (shamelessly stolen from gtk.m4 and then hacked around
#   a fair amount)
#
# LAST MODIFICATION
#
