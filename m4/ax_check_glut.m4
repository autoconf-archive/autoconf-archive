...
#   the configuration would export the required compiler flags to "GLUT_CFLAGS"
#   and "GLUT_LIBS" environment variables. These two variables can also be
#   overwritten by defining the environment variables before executing the
#   configure program. If it was predefined, configure would not try to
#   overwrite it, but it would still perform the compile and link test. Only
#   when the tests succeeded does the configure script to export "HAVE_GLUT=1"
#   and to run "ACTION-IF-FOUND" hook.
#
#   If user didn't specify the "ACTION-IF-FOUND" hook, the configuration would
#   prepend "GLUT_CFLAGS" and "GLUT_LIBS" to "CFLAGS" and "LIBS", like many other
#   autoconf macros do.
#
