##### http://autoconf-archive.cryp.to/ac_subst_prefix_subpaths.html
#
# OBSOLETE MACRO
#
...
#   $prefix/$exec_prefix
#
#   If the path is indeed relative to $prefix/$exec_prefix, then a
#   single "./" (dotslash) is prepended, otherwise it can be seen as an
#   absolute path that can not be moved, which you possibly do for
#   "/etc" files, or even those ending up in "/lib/modules" or
#   "/winnt/system".
#
#   this macro is not very intelligent, it's just a first try in this
#   direction. It does currently just look into the current patterns,
#   and replaces a ${prefix} with a simple dot. Amazingly, it works
#   quite well for most packages.
#
...
#   either ${prefix} or ${exec_prefix}. Just cut out the prefix and
#   ac_define the value. The value is uppercased and PATH_ prepended
#   ie. ac_define_sub_paths(bindir libdir pkgdatadir) will create the
#   defines PATH_BINDIR PATH_LIBDIR PATH_PKGDATADIR - see posix'
